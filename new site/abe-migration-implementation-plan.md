...
1. **[COW] Fact ledger.** Open a per-page ledger. Verify against `.gov.au` (or the internal verified register) with dates: regulator, legislation, permit/approval thresholds, fees, eligibility. Confirm internal facts against LearnWorlds/Notion (price, pass mark, attempts, modules, hours, reviewer) — never inferred from a sibling state. No figure is written without a source the moment it is written (mistakes-log M4). **Provenance gate, added 4 Aug 2026 — second occurrence, the trigger ROADMAP rule 3 authorises: NSW cost a complete page built on a fact that reversed at Stage 9 (`skill-reviews/facts/2026-08-01-nsw-git-conditions-and-fees.md`); QLD cost only a fraction, because the reading came first.** Before moving to Stage 3, confirm every row the ledger will publish is **regulator-sourced**. The register now labels rows by provenance — `kb/register/online-delivery-policy-by-state.md` §4 splits "Primary sources (regulator documents)" from industry-guide "Secondary sourcing", and any register row can carry an explicit `UNVERIFIED` marker. A row resting only on an industry guide, or marked `UNVERIFIED`, does not go to Stage 3 on the strength of the guide — send it to a facts session first, and wait for the read.
...
| W3-6 | [CC] | **White Card hub** `/white-card` (10.8k impr already on the slug). | B | W3-1..W3-5 | Five state cards + `ItemList`; cross-links to `/faq`, `/accreditation`, `/experts`. **Correction, 2 Aug 2026: this dependency is only half-real.** `hubs.spokes` has no minimum length and `comparison.columns[].soon` already renders a non-linked "Coming soon" cell, so the hub does not need all five spokes built first — only the ones it names as built. Do not block starting W3-6 on W3-5 finishing. |

**Wave 3 exit gate:** five spokes + hub live; ASQA / `PartnerDisclosure` hardened. **Half met, 7 Aug
2026: all five spokes + the hub are live.** The hardening half is **not** met — `SourcesFooter.astro`'s
`asqa` prop is never threaded through `HubLayout.astro`, so the hub's sitewide compliance line renders
the wrong (state-approved-direct) disclosure sentence for an asqa-accredited page. Cross-session
(`design` fixes the component, `skills` fixes the schema gap); tracked in
`pipeline/white-card/07-verification.md` §7, not yet routed to either.
...
1. ~~**[AP]** Raise + resolve the LearnWorlds ticket~~ **DONE — confirmed resolved by Andrey, 16 Aug 2026.** The school can move to `learn.abeeducation.edu.au`. **This was the one external blocker and it is closed**, so nothing in this runbook now waits on a third party. Andrey also directed that the **checkout paths stay AS-IS**: enrol CTAs keep their same-origin `/payment?product_id=…` form rather than being rewritten to `learn.*`, which holds because `public/_redirects` already 301s `/payment*`, `/course/*`, `/program/*` and `/bundle/*` to the subdomain. **Still open, and now ABE-side rather than external:** the learn.* indexation decision and the GSC property-type check (both R6, step 1a below), plus the unverified question at `public/_redirects:178` of whether `?product_id=` survives the 301.
1a. **[AP/CC]** Decide learn.* indexation and confirm the GSC property is a **domain** property (R6). Was bundled into the ticket above; it is a decision ABE makes, not one LearnWorlds answers, so it survives that ticket's closure and is still due **before** step 3.
...
**Wave 3** — ☑ W3-1 ◪ W3-2 ◪ W3-3 ◪ W3-4 ◪ W3-5 ◪ W3-6 *(**W3-5 `/white-card-act` and the W3-6 hub
redesign both built 7 Aug 2026**, commit `f10f166` — AlertForce 91826, ◪ not ☑ for the same reason as
NSW/TAS/QLD: no confirmed `buyUrl`, every CTA is the in-page `#enrol` anchor, recorded in the page's
own frontmatter comment. The hub's comparison table was reworked to match `/owner-builder-courses`
exactly (two shared-CSS regressions found and fixed the same session, `kb/mistakes-log.md` row 25);
a fresh-subagent Stage 7 pass on the hub then found and fixed two more defects (TAS/ACT "Issued by"
missing its "Statement of Attainment" qualifier, and a stale `reviewedBy` date) — full record in
`pipeline/white-card/07-verification.md`'s "Re-verification · 7 August 2026" section. **The TAS
"Tasmanian residents only" contradiction flagged unresolved just below (at the 3 Aug entry) is now
fully fixed, 7 Aug 2026:** a facts session read WorkSafe Tasmania's own wording plus Blue Dog's
training.gov.au scope (`skill-reviews/facts/2026-08-07-tas-residency-fix-blue-dog-scope.md`) and found
no source supports a residency test — only a "completed in Tasmania" condition — then a build session
corrected all 11 locations across `white-card-tas.mdx`, `faqs-white-card-tas.ts`, `white-card-nsw.mdx`
and `faqs-white-card-nsw.ts` (commit `31b52c4`), and a skills session corrected the matching row in
`kb/rules/authority-model.md` (commit `57cfaaf`). **W3-3 `/white-card-qld` built 3 Aug 2026**,
commit `b36d8b4`, ◪ not ☑ for the same reason as NSW/TAS: no confirmed `buyUrl`, every CTA is the
in-page `#enrol` anchor. Delivered as a genuinely live Connected Real Time Delivery (CRTD) session
with Blue Dog Training — not self-paced, a materially different shape from WA/TAS's self-paced model —
correcting a live competitor's superseded "100km rural exception" claim with WHSQ's own Conditions of
Agreement V6.1 and the Queensland Training Ombudsman's review. Stage 9 graded independently: **Amber**,
`correct_and_safe: amber`, `passed_gates_first_time: amber`. Not for a regulatory error — the authority
model is correct throughout (Blue Dog credited, ABE never claimed as an RTO or WHSQ-approved) — but for
two process gaps: `check-claims.mjs` was never run at Stage 7 (fixed with an addendum once caught), and
the build had to edit design-owned `SiteHeader.astro` to satisfy the orphan-page guardrail, the
**third sighting** of that exact crossing (see the Session-type path check row in `ROADMAP.md`'s Phase
3 table) and the first not disclosed in the run's own artefacts until an independent grader found it via
`git status` — since fixed by disclosing it explicitly in the pipeline record. **A price was corrected
mid-build**: Andrey's initial $99 (a sibling-price default) was superseded by Blue Dog's real weekly
CRTD timetable, $109 Mon-Fri / $169 Saturday, supplied with the full schedule; every occurrence across
the page was found and corrected, confirmed zero stray `$99` in the built output. Artefacts
`pipeline/white-card-qld/01`–`07`; review
`skill-reviews/2026-08-03-abe-course-page-astro-white-card-qld.md`. **A 3 Aug facts session closed the
last delivery-mode gaps for TAS, ACT and WA the same day** — all five ABE White Card states now have a
regulator-sourced delivery-mode row in `online-delivery-policy-by-state.md`; TAS's own page copy
("Tasmanian residents only") was found unsourced to either the regulator or the Regulations, flagged
`[build]` — **fixed 7 Aug 2026, see the W3-5/W3-6 entry above.** That session also unblocked both
W3-5 and W3-6.
**W3-2 `/white-card-nsw` built 1 Aug 2026**, PR #103, ◪ not ☑ for the same reason as TAS: it ships indexable with **no purchase path**, because there is no NSW White Card product in LearnWorlds at all, so every CTA is the in-page `#enrol` anchor. Both image slots are FPO placeholders. The two things that made it buildable were verified at source, not carried in: CPCWHS1001 is on RTO 45708's scope to deliver and assess through 2030, and Upskill holds SafeWork NSW registration **RTO800520** for General Construction Induction Training, which NSW requires on top of ASQA registration. Stage 9 graded independently: Amber, `correct_and_safe: amber`, `passed_gates_first_time: red`. **Two blockers were raised mid-build and cleared by Andrey** — the delivery-mode question (SafeWork NSW's Oct 2022 Specific Conditions bar online learning for GIT; that document is outdated, and no current one could be located publicly, so the page attributes live delivery to the RTO's registration rather than to the regulator) and a cost section that credited competitors with excluding a card fee the RTO actually pays. **A side effect worth knowing before W2-3:** noindexing the on-hold `/owner-builder-nsw-course` in the same PR turned `/nsw-owner-builder-course` into a redirect into a dead end, and it is now PENDING in `check-redirect-targets`. Those two URLs carry 38,257 impressions between them and their cutover fate is an open commercial decision. Earlier rows ticked 28 Jul against `dist/` and the pushed commit, not against memory. **W3-1 `/white-card-wa` is built, indexable and shipped to staging** — the largest White Card asset at 39.9k impressions, position 9.04. Its `buyUrl` resolves, which is why it ships indexable where W3-4 does not. Two cutover gates remain and neither blocks staging: two generated images (both slots render FPO placeholders today) and confirmation that **`/payment` is served at the deploy origin** — it is live on the legacy origin but absent from the Worker's asset set, so all four CTAs are dead on staging. Artefacts `pipeline/white-card-wa/01`–`07`; review `skill-reviews/2026-07-28-abe-course-page-astro-white-card-wa.md`, Amber, independent grader, red on `passed_gates_first_time`. **W3-4 `/white-card-tas` was published 28 Jul** on Andrey's call and is now indexable, carrying its missing checkout as a **warning rather than a blocker**: TAS payment is not configured, so every CTA is the in-page `#enrol` anchor and the page cannot be bought from. It earns that because the legacy URL holds real equity (7,092 impressions, position 11.81). ⚠️ **Warning, not a blocker (Andrey, 28 Jul): the page states $59 and a live checkout charges A$39** (`/payment?product_id=white-card-tas&type=course`, corroborated by LearnWorlds at A$117 over 3 payments) — either the $39 product is legacy and should be retired, or the page is wrong [AP]. Built 23 Jul as evidence run 2, the first exercise of the asqa-accredited authority model; its two photos and all three RTO contacts are done. It also forced the ASQA authorship correction now enforced in `guardrails.ts`: on an accredited course the RTO is the developer, never an ABE person. **W3-6 the hub does not need every spoke built first, found 2 Aug 2026** — the `hubs` schema types `spokes[].course` as a `reference('courses')`, which does fail Zod if a *named* spoke doesn't exist, but `spokes` itself has no minimum length and `comparison.columns[].soon` already renders an unlinked "Coming soon" cell for a not-yet-built state. Four of five spokes exist as of 3 Aug (WA, NSW, TAS, QLD); only ACT (W3-5) is outstanding, and the hub can ship naming ACT as "Coming soon" rather than waiting for it. Both W3-5 and W3-6 are unblocked as of 3 Aug 2026 — neither is built yet.)*
