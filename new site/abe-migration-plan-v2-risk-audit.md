...
**Status, 16 Aug 2026 — the ticket is closed; R6 is NOT.** Andrey confirmed the LearnWorlds subdomain ticket resolved, so the move to `learn.*` is agreed and the external half of this risk is gone. **The two decisions R6 exists for are ABE's own and remain open:** whether learn.* course pages should be indexable at all, and whether the GSC property is a domain property. A resolved ticket is not a decided indexation strategy, and this risk stays live until those two are answered. Separately, the confirmation directed that checkout paths stay AS-IS on the marketing host: enrol CTAs keep `/payment?product_id=…` and are carried to learn.* by `public/_redirects`, so this risk's scope does not extend to rewriting them.
