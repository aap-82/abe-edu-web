...
2. Move LearnWorlds to `learn.abeeducation.edu.au` (feasibility **confirmed 16 Aug 2026**, §10 item 3 — no longer a pre-step); keep it serving course delivery and checkout. The learn.* indexation decision (R6) is still open and is made before this move. Enrol CTAs keep their same-origin `/payment?product_id=…` form and are carried by the redirect map; they are not rewritten to learn.* (Andrey, 16 Aug).
...
3. **LearnWorlds subdomain confirmation — answered 16 Aug:** the ticket is **confirmed resolved**. It was the cutover's only external dependency and it is closed. Andrey also directed that the **checkout paths stay AS-IS** and are not to be treated as a blocker: the enrol CTAs keep their same-origin `/payment?product_id=…` form rather than being rewritten to `learn.*`, which is safe because `public/_redirects` already 301s `/payment*`, `/course/*`, `/program/*` and `/bundle/*` to `learn.abeeducation.edu.au`. Two things this does **not** close, both ABE-side rather than external: the **learn.\* indexation decision** and the **GSC property-type check** (R6 below), and the note at `public/_redirects:178` that LearnWorlds re-applies product query params, so whether `?product_id=` survives the 301 is still unverified.
