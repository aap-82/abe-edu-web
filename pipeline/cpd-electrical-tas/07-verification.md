...
- member row 3 of 11: `TAS CPD: Cyber Risks and Workplace Safety (1 pt)` -> `Cyber Risks and Workplace Safety`
...
- card renders in full: `Cyber Risks and Workplace Safety | Protecting client data and connected site systems from common threats. | 1 CPD point | 47 min average` — blurb and measured time both preserved
...
This page remains `noindex` for its own unrelated blocker (no 2026 electrician checkout id — the
`buyUrl` is still a `TBC-` placeholder). Nothing here changes that.
