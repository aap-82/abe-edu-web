...
3 step bodies in the "how it works" ladder changed on this page:
  ...
  - "Attend the classroom session and pass"
  - "AlertForce issues your Statement of Attainment"
  - "Access Canberra posts your card"
...
One rewrite dropped the words "Your RTO partner" from the AlertForce step, leaving "AlertForce
(RTO 91826) issues your Statement of Attainment". Checked against `kb/rules/asqa-disclosure-
framework.md`: the seven mandated per-course disclosure locations are the hero inline line, the
near-CTA template, the per-course footer legal block and the footer copyright bar - a step body is
not one of them, and the RTO name and number both survive in the sentence. No disclosure location
lost content. Recorded because the phrase change is real, not because it breaches anything.
...
```
before: Enrol → Attend the classroom session → Pass the assessment → Apply for your card
after:  Enrol → Attend|the classroom session → Pass|the assessment → Apply|for your card
```
