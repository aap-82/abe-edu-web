...
**Measured after the change** (`dist/white-card-wa/index.html`): `/Organisation/Details/31193` ×9,
...
(`git show 7ea0300 -- src/content/courses/white-card-wa.mdx`). The middle "White Card" breadcrumb
...
`breadcrumb` array changed; every fact re-verified across this file's many prior passes (the Form 75
pathways, the doubled "against", the two image slots) is untouched.

## Measured

| Check | Measured value |
|---|---|
| `breadcrumb[]` length | **3** (was 2) — Home, White Card, White Card WA |
...
| `BreadcrumbList` JSON-LD | 3 items: Home, `White Card -> https://www.abeeducation.edu.au/white-card`, White Card WA |
...
this page in particular has already had that full pass, several times over, above.

## Ship decision

**Merge-ready.** The only change since the last verification entry is confirmed cosmetic-structural
...
3 days later than it should have, and recorded as such rather than backdated. **B4's successor
gates (F16, the `/payment` origin) remain open and are unchanged by this pass.**
...
As with white-card-qld, these four describe COURSE CONTENT and carry no fee, threshold, date,
licence class or regulator obligation. Connecting words only.
...
```
before: Enrol → Work through the theory online → Book your live assessment → Blue Dog issues your card
after:  Enrol → Work through|the theory online → Book|your live assessment → Blue Dog issues your card
```
