...
## Demand list
Tag every item: [skills] | [design] | [facts] | [build] — one per session type. Anything else is
reported UNROUTED by `scripts/demand-split.mjs` rather than dropped, so the handover notes can be
derived.
