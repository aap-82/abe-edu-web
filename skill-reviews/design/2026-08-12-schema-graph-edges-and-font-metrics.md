...
- [build] `1c26fab` ("content(steps): split every step body...") changed 14 files under `src/content/` without re-running Stage 7, putting `check-pipeline.mjs` into **8 FAIL** — `white-card-act`, `white-card-qld`, `white-card-tas`, `white-card-wa`, `wa-owner-builder-course` and the three `cpd-*-tas` bundles. Its predecessor `c7c6c43` re-verified five pages for the first half of the same edit; this half was not. Re-run Stage 7 for the eight, or revert. This is a live publish blocker, not a warning.
